...
- 🌐 **Translation** — a localized `docs/` set, in a sibling folder `docs-<lang>/` (e.g. `docs-tr/`, `docs-es/`) mirroring the same `01`–`09` numbering and filenames as `docs/`, so cross-references stay valid across languages.
- 📄 **New topic doc** — if it doesn't fit an existing `docs/01`–`09` file, add it as `docs/10-<topic>.md` (next free number, kebab-case) and link it from `docs/README.md`; don't fold unrelated topics into an existing numbered file just to avoid adding one.
